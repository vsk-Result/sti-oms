...
@section('toolbar-title', 'Загрузка из истории оплат')
@section('breadcrumbs', Breadcrumbs::render('payment_imports.types.history.create'))
            ...
            <div class="card mb-5 mb-xl-8">
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">Загрузка из истории оплат</span>
                    </h3>
                </div>
                <div class="card-body py-3">
                    ...
                    <form class="form" action="{{ route('payment_imports.types.history.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="scroll-y me-n7 pe-7" id="kt_modal_new_address_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_new_address_header" data-kt-scroll-wrappers="#kt_modal_new_address_scroll" data-kt-scroll-offset="300px">
                            <div class="row mb-5">
                                <div class="col-md-12 fv-row">
                                    <label class="required fs-5 fw-bold mb-2">Объект</label>
                                    <select name="object_id" class="form-select form-select-solid" data-control="select2">
                                        @foreach($objects as $object)
                                            <option value="{{ $object->id }}">{{ $object->code . ' ' . $object->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
