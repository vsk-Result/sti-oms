...
@section('title', 'История БГ и депозитов')
@section('toolbar-title', 'История БГ и депозитов')
@section('breadcrumbs', Breadcrumbs::render('bank_guarantees.history.index'))

@section('content')
    @include('bank-guarantees.history.modals.filter')
            ...
            <div class="card-header border-0 pt-6">
                ...
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                        <button type="button" class="btn btn-primary me-3" data-bs-toggle="modal" data-bs-target="#filterBankGuaranteesHistoryModal">
                    ...
                    <table class="table table-hover align-middle table-row-dashed fs-6">
                        <thead>
                            <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-150px">Банк. гарантия</th>
                                ...
                                @foreach($audit->getModified() as $fieldName => $changes)
                                    ...
                                    @if (array_key_exists($fieldName, $fieldsArray))
                                        <tr>
                                            ...
                                            <td>
                                                @if ($audit->auditable)
                                                    <a href="{{ route('bank_guarantees.edit', $audit->auditable) }}">БГ #{{ $audit->auditable->id }}</a>
                                                @else
                                                    БГ #{{ $audit->auditable_id }}
                                                @endif
                                            </td>
                                            ...
                                            @else
                                                ...
                                                <td>
                                                    @if ($fieldName === 'object_id' && isset($changes['old']))
                                                        {{ $objects[$changes['old']] ?? '' }}
                                                    @elseif ($fieldName === 'contract_id' && isset($changes['old']))
                                                        {{ $contracts[$changes['old']] ?? '' }}
                                                    ...
                                                    @elseif ($fieldName === 'bank_id' && isset($changes['old']))
                                                        {{ $banks[$changes['old']] ?? '' }}
                                                    @elseif (($fieldName === 'organization_id') && isset($changes['old']))
                                                    ...
                                                    @elseif ($fieldName === 'amount_deposit')
                                                        <span class="{{ ($changes['old'] ?? 0) < 0 ? 'text-danger' : 'text-success' }}">{{ number_format($changes['old'] ?? 0, 2, '.', ' ') }}</span>
                                                    @else
                                                        {{ $changes['old'] ?? '' }}
                                                    @endif
                                                ...
                                                <td>
                                                    @if ($fieldName === 'object_id' && isset($changes['new']))
                                                        {{ $objects[$changes['new']] ?? '' }}
                                                    @elseif ($fieldName === 'contract_id' && isset($changes['new']))
                                                        {{ $contracts[$changes['new']] ?? '' }}
                                                    ...
                                                    @elseif ($fieldName === 'bank_id' && isset($changes['new']))
                                                        {{ $banks[$changes['new']] ?? '' }}
                                                    @elseif (($fieldName === 'organization_id') && isset($changes['new']))
