    ...
    <div class="card-header border-0 pt-6">
        ...
        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                ...
                <form action="{{ route('acts.exports.store') . (strpos(request()->fullUrl(), '?') !== false ? substr(request()->fullUrl(), strpos(request()->fullUrl(), '?')) : '') }}" method="POST" class="hidden">
                    @csrf
                    <a
                            href="javascript:void(0);"
                            class="btn btn-bg-light btn-color-success me-3"
                ...
                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-200px py-4" data-kt-menu="true">
                    <div class="menu-item px-3">
                        <a href="{{ route('objects.reports.act_category.index', $object) }}" class="menu-link px-3">
                            Отчет по актам
                        </a>
                    </div>
                </div>
            ...
            <table class="table table-hover align-middle table-row-dashed fs-7">
                ...
                <tbody class="text-gray-600 fw-bold">
                    <tr class="fw-bolder" style="background-color: #fbfbfb; color: #737373;">
                        ...
                        <td colspan="6" class="ps-4" style="vertical-align: middle;">Итого (неизменяемая часть)</td>
                        <td>
                    ...
                    @forelse($fixActs as $act)
                        <tr class="collapse-row" data-trigger="fix-acts" style="display: none;">
                            <td></td>
                            <td class="ps-4">
                                @if(auth()->user()->can('show objects'))
                                    <a href="{{ route('objects.acts.index', $act->object) }}" class="show-link" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $act->object->name }}">{{ $act->object->code }}</a>
                                @else
                                    <span data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $act->object->name }}">{{ $act->object->code }}</span>
                                @endif
                            </td>
                            <td class="px-3">
                    ...
                    <tr class="fw-bolder" style="background-color: #fbfbfb; color: #737373;">
                        ...
                        <td colspan="6" class="ps-4" style="vertical-align: middle;">Итого (изменяемая часть)</td>
                        <td>
                    ...
                    @forelse($floatActs as $act)
                        <tr class="collapse-row" data-trigger="float-acts" style="display: none;">
                            <td></td>
                            <td class="ps-4">
                                @if(auth()->user()->can('show objects'))
                                    <a href="{{ route('objects.acts.index', $act->object) }}" class="show-link" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $act->object->name }}">{{ $act->object->code }}</a>
