            ...
            <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6">
                <a href="javascript::void(0);">
                    <div class="d-flex align-items-center">
                        <div class="fs-4 fw-bolder {{ $totalInfo['amount'] >= 0 ? 'text-success' : 'text-danger' }}">{{ number_format($totalInfo['amount'], 2, '.', ' ') }}</div>
                    </div>
                    <div class="fw-bold fs-6 text-gray-400">Итого списано</div>
                </a>
            </div>

            <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-6">
                <div class="d-flex align-items-center">
                    <div class="fs-4 fw-bolder">{{ number_format($writeoffs->total(), 0, '.', ' ') }}</div>
                </div>
                <div class="fw-bold fs-6 text-gray-400">Списаний</div>
            </div>
        </div>

        <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                <button type="button" class="btn btn-primary me-3" data-bs-toggle="modal" data-bs-target="#filterWriteoffModal">
                ...
                <form action="{{ route('writeoffs.exports.store') . (strpos(request()->fullUrl(), '?') !== false ? substr(request()->fullUrl(), strpos(request()->fullUrl(), '?')) : '') }}" method="POST" class="hidden">
                    @csrf
                    <a
                ...
                <tbody class="text-gray-600 fw-bold">
                    @forelse($writeoffs as $writeoff)
                        <tr>
                            ...
                            <td>{{ $writeoff->crm_employee_uid }}</td>
                            <td>{{ $writeoff->description }}</td>
                            <td>
                                <span class="{{ $writeoff->amount >= 0 ? 'text-success' : 'text-danger' }}">{{ $writeoff->getAmount() }}</span>
                            </td>
                            <td>
                                @if ($writeoff->hasMedia())
                                    <div class="d-flex flex-column gap-4">
                                        @foreach($writeoff->getMedia() as $media)
                                            <div class="d-flex flex-aligns-center pe-10">
                                                <img alt="" class="w-30px h-30px me-3" src="{{ get_extension_image_path($media->getExtensionAttribute()) }}">

                                                <div class="ms-1 fw-semibold">
                                                    <a
                                                            target="_blank"
                                                            download="Списание #{{ $writeoff->id }} от {{ $writeoff->getDateFormatted() }}"
                                                            href="{{ $media->getUrl() }}"
                                                            class="fs-8 text-hover-primary fw-bold"
                                                    >
                                                        {{ $media->file_name }}
                                                    </a>

                                                    <div class="text-gray-400 fs-9">{{ $media->human_readable_size }}</div>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-150px py-4" data-kt-menu="true">
                                        @can('edit writeoffs')
                                            <div class="menu-item px-3">
                                                <a href="{{ route('writeoffs.edit', $writeoff) }}" class="menu-link px-3">Изменить</a>
                                            </div>
...
{{--                                            @endif--}}
                                            ...
                                            <div class="menu-item px-3">
                                                <form action="{{ route('writeoffs.destroy', $writeoff) }}" method="POST" class="hidden">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a
                                                            href="javascript:void(0)"
                                                            class="menu-link px-3 text-danger"
                                                            onclick="event.preventDefault(); if (confirm('Вы действительно хотите удалить списание?')) {this.closest('form').submit();}"
                                                    >
                                                        Удалить
                                                    </a>
                                                </form>
