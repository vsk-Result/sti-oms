...
@section('toolbar-title', 'Выписки')
@section('breadcrumbs', Breadcrumbs::render('statements.index'))
                ...
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                        @can('create statements')
                            <a href="{{ route('statements.create') }}" class="btn btn-light-primary">
                        ...
                        <tbody class="text-gray-600 fw-bold">
                            @forelse($statements as $statement)
                                <tr>
                                    ...
                                    <td class="fw-bolder {{ $statement->incoming_balance >= 0 ? 'text-success' : 'text-danger' }}">{{ $statement->getIncomingBalance() }}</td>
                                    <td class="text-danger fw-bolder">{{ $statement->getAmountPay() }}</td>
                                    <td class="text-success fw-bolder">{{ $statement->getAmountReceive() }}</td>
                                    <td class="text-success fw-bolder {{ $statement->outgoing_balance >= 0 ? 'text-success' : 'text-danger' }}">{{ $statement->getOutgoingBalance() }}</td>
                                    <td class="text-dark fw-bolder">{{ $statement->createdBy->name }}</td>
                                    <td class="text-dark fw-bolder">@include('partials.status', ['status' => $statement->getStatus()])</td>
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                            @can('show statements')
                                                <div class="menu-item px-3">
                                                    <a href="{{ route('statements.show', $statement) }}" class="menu-link px-3">Посмотреть</a>
                                                </div>
                                                <div class="menu-item px-3">
                                            ...
                                            @can('edit statements')
                                                <div class="menu-item px-3">
                                                    <a href="{{ route('statements.edit', $statement) }}" class="menu-link px-3">Изменить</a>
                                                </div>
