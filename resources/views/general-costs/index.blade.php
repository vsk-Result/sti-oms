...
@section('title', 'Распределение общих затрат')
                            ...
                            <tr>
                                ...
                                @php
                                    ...
                                    foreach($generalCostsInfo['generalInfo'] as $year => $infoArray) {
                                        ...
                                        foreach($infoArray as $info) {
                                            $totalCuming += \App\Models\Object\BObject::find($object->id)->payments()->where('payment_type_id', \App\Models\Payment::PAYMENT_TYPE_NON_CASH)->where('amount', '>=', 0)->whereIn('company_id', [1, 5])->whereIn('organization_sender_id', $object->customers->pluck('id')->toArray())->sum('amount');
