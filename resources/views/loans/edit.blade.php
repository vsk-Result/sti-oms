        ...
        <div id="kt_content_container" class="container">
            <div class="card mb-5 mb-xl-8">
                ...
                <div class="card-body py-3">
                    <form class="form" action="{{ route('loans.update', $loan) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row mb-5">
                            <div class="col-md-12 fv-row">
                                            ...
                                            <div class="position-relative mb-3">
                                                <select name="organization_type_id" data-control="select2" class="form-select form-select-solid form-select-lg">
                                                    @foreach($organizationTypes as $typeId => $typeName)
                                                        <option value="{{ $typeId }}" {{ $loan->organization_type_id === $typeId ? 'selected' : '' }}>{{ $typeName }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="position-relative mb-3">
                                                <select name="credit_type_id" data-control="select2" class="form-select form-select-solid form-select-lg">
                                                    @foreach($creditTypes as $typeId => $typeName)
                                                        <option value="{{ $typeId }}" {{ $loan->credit_type_id === $typeId ? 'selected' : '' }}>{{ $typeName }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                ...
                                <div class="row">
                                            ...
                                            <div class="mb-1">
                                                ...
                                                <div class="position-relative mb-3">
                                                    <select name="auto_paid" data-control="select2" class="form-select form-select-solid form-select-lg">
                                                        <option value="auto" {{ $loan->is_auto_paid ? 'selected' : '' }}>Автоматический</option>
                                                        <option value="manual" {{ !$loan->is_auto_paid ? 'selected' : '' }}>Ручной</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-8 fv-row" id="visible-search-name" style="{{ !$loan->is_auto_paid ? 'display: none;' : '' }}">
                                        <div class="fv-row">
                                            <div class="mb-1">
...
@push('scripts')
    <script>
        $(function() {
            ...
            const value = $('select[name=auto_paid]').val();
            if (value === 'auto') {
