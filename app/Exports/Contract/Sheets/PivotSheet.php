    ...
    ShouldAutoSize
{
    ...
    public function styledForCUMCHANKA(Worksheet $sheet): void
    {
        $total = $this->total;
        $THINStyleArray = [ 'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['argb' => '0000000'],],],];
        ...
        $sheet->setCellValue('A' . 1, 'Справка объекта 363 | Камчатка (МАПКЕ)' . ' на ' . now()->format('d.m.Y'));
        $sheet->mergeCells('A1:M1');
        ...
        $row++;
        foreach ($this->contracts as $contract) {
            if (! str_contains($contract->name, 'МАПКЕ')) {
                continue;
            }
        ...
        $row++;
        foreach ($this->contracts as $contract) {
            if (! str_contains($contract->name, 'КЛЦ')) {
                continue;
            }
