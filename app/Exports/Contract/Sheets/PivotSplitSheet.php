    ...
    ShouldAutoSize
{
    ...
    public function __construct(BObject $object, Collection $contracts, array $total, ?string $name = null)
    {
        $this->object = $object;
        ...
        foreach ($currencies as $currency) {
            foreach ($this->contracts as $contract) {
                ...
                if (count(array_unique($contract->avanses->pluck('type_id')->toArray())) > 1 || count(array_unique($contract->avansesReceived->pluck('type_id')->toArray())) > 1) {
                    $sheet->setCellValue('A' . $row, '        Фиксированный аванс');
                    ...
                    $sheet->setCellValue('A' . $row, '        Целевой аванс');
                    $sheet->setCellValue('B' . $row, '');
                    $sheet->setCellValue('C' . $row, '');
    ...
    public function stylesForCUMCHANKA(Worksheet $sheet): void
    {
        ...
        $sheet->setCellValue('A' . 1, 'Справка объекта 363 | Камчатка (' . $this->name . ')' . ' на ' . now()->format('d.m.Y'));
        $sheet->mergeCells('A1:M1');
        ...
        foreach ($this->contracts as $contract) {
            if ($this->name === 'МАПКЕ' && ! str_contains($contract->name, 'МАПКЕ')) {
                continue;
            }

            if ($this->name === 'Кольцово' && ! str_contains($contract->name, 'КЛЦ')) {
                continue;
            }
