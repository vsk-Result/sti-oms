...
class AllDebtsProviderSheet implements
    WithTitle,
    WithStyles,
    ShouldAutoSize
{
    ...
    public function styles(Worksheet $sheet): void
    {
        ...
        foreach ($this->object_ids as $object_id) {
            ...
            $providerDebts = $this->pivotObjectDebtService->getPivotDebts($object_id, PivotObjectDebt::DEBT_TYPE_PROVIDER, ['with_sorted_details' => true]);
            foreach ($providerDebts['organizations'] as $organization) {
...
//        }
        ...
        foreach ($final_array as $record) {
            ...
            $sheet->setCellValue('C' . $row, $record['amount_fix']);
            $sheet->setCellValue('D' . $row, $record['amount_float']);
            $sheet->setCellValue('E' . $row, $record['total_amount']);
