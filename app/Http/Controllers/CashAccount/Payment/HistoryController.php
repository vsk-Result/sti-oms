...
{
    public function index(Request $request, CashAccount $cashAccount): View
    {
        $fields = [
            'category' => 'Категория',
            'company_id' => 'Компания',
        ...
        $auditables = [
            CashAccountPayment::class => 'Оплаты'
        ];
        $auditable = CashAccountPayment::class;
        ...
        $query->where('auditable_type', CashAccountPayment::class);
        ...
        return view(
            'cash-accounts.payments.history.index',
            compact(
                'audits', 'events', 'fields', 'objects', 'statuses',
                'companies', 'organizations', 'users', 'auditables', 'auditable', 'cashAccount'
            )
        );
    }
}
