...
class SplitNDFLController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $paymentsForSplit = Payment::where('type_id', Payment::TYPE_GENERAL)
            ->where('company_id', 1)
            ->whereBetween('date', ['2022-01-01', '2022-02-02'])
            ->where('description', 'NDFL')
            ->orderBy('amount')
            ->get();

        $employeesCount = [];
        $crm = Workhour::select(['id', 'date', 'o_id', 'e_id'])->where('date', 'LIKE', '2022-01' . '%')->get()->groupBy('o_id');
        ...
        foreach ($employeesCount as $code => $count) {
            ...
            $requestData['description'] = 'Налог на доходы физических лиц за январь 2022';
