...
class ImportWorkersSalaryDebtsFrom1CExcel extends HandledCommand
{
    protected $signature = 'oms:import-workers-salary-debts-from-1c-excel';

    protected $description = 'Загружает долги по зарплатам рабочих из 1С из Excel';
    ...
    public function handle()
    {
        ...
        try {
            $response = Http::withBasicAuth('WebService', 'Vi7je7da')->post('https://1c.st-ing.com/prod_STI_ushr/hs/Telebot/Universal', [
                'Method' => 'GetPayDebtWorkers',
            ]);

            $data = json_decode($response->getBody()->getContents(), true);

            $workersSalaryInfo = $data['Result'];
        } catch (\Exception $e) {
            $this->sendErrorMessage('Не удалось загрузить долги по зарплатам рабочих из 1С');
        ...
        foreach ($workersSalaryInfo as $info) {
        ...
        Cache::put('workers_salary_1c_data', $formattedInfo);

        $this->sendInfoMessage('Долги по зарплатам рабочих успешно загружены');
