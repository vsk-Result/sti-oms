...
class UpdatePaymentCodes extends HandledCommand
{
    protected $signature = 'oms:update-payment-codes';

    protected $description = 'Исправляет статья затрат оплат';
    ...
    public function handle()
    {
        ...
        foreach ($newCodes as $oldCode => $newCode) {
            ...
            $this->sendInfoMessage('Найдено ' . Payment::whereIn('code', [$oldCode, ' ' . $oldCode, '  ' . $oldCode, $oldCode . ' ', $oldCode . '  '])->count() . ' оплат');

            Payment::whereIn('code', [$oldCode, ' ' . $oldCode, '  ' . $oldCode, $oldCode . ' ', $oldCode . '  '])->update([
                'code' => $newCode . '***'
            ]);
        ...
        foreach ($newCodes as $oldCode => $newCode) {
            ...
            $this->sendInfoMessage('Найдено ' . Payment::where('code', $newCode . '***')->count() . ' оплат');

            Payment::where('code', $newCode . '***')->update([
                'code' => $newCode
            ]);
