...
class ObjectFilesService
{
    ...
    public function uploadObjectFile(BObject $object, UploadedFile $file): void
    {
        ...
        if (str_contains($file->getClientOriginalName(), 'распред')) {
            $importData = Excel::toArray(new RegistryImport(), $file);

            if (isset($importData['для ОМСпо категориям'])) {
                ...
                foreach ($importData['для ОМСпо категориям'] as $rowIndex => $row) {
                    if ($rowIndex === 0 || empty($row[0])) {
                        continue;
                    }
                    $paymentDate = $row[0] ?? '';
                    $senderName = $row[1] ?? '';
                    $senderInn = empty($row[2]) ? null : $row[2];
                    $receiverName = $row[3] ?? '';
                    $receiverInn = empty($row[4]) ? null : $row[4];
                    $paymentAmount = $this->sanitizer->set($row[5] ?? 0)->toAmount()->get();
                    $paymentDescription = $this->sanitizer->set($row[6] ?? '')->upperCaseFirstWord()->get();
                    $objectCode = $row[7] ?? '';
                    $paymentCode = $row[8] ?? '';
                    $paymentCategory = empty($row[9]) ? Payment::CATEGORY_MATERIAL : $row[9];
                    ...
                    $payment = $this->paymentService->createPayment([
                        ...
                        'date' => is_string($paymentDate)
                            ? Carbon::parse($paymentDate)->format('Y-m-d')
                            : Carbon::parse(Date::excelToDateTimeObject($paymentDate))->format('Y-m-d'),
                        'amount' => $paymentAmount,
                        'parameters' => [],
                        'amount_without_nds' => $paymentAmount,
                        'is_need_split' => false,
                        'was_split' => false,
                        'status_id' => Status::STATUS_ACTIVE,
                        'currency_rate' => 1,
                        'currency_amount' => $paymentAmount,
                    ]);
