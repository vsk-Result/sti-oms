...
class CalculateWorkersCostService
{
    const COMPANY_GROUPS = [
        ...
        'Транспорт (перевозка сотрудников) + Авиабилеты' => '5.11;5.11.1;5.11.2;5.12;7.14;7.15',
    ...
    const OBJECTS_GROUPS = [
        ...
        'Транспорт (перевозка сотрудников) + Авиабилеты' => '5.11;5.11.1;5.11.2;5.12',
        'Спецодежда и СИЗ' => '5.8',
        'Банковские расходы (ком.БГ, обсл.спец.сч)' => '7.6;7.7.1',
                ...
                foreach ($quart['months'] as $month) {
                    ...
                    foreach ($objects as $object) {
                        ...
                        if (isset($workhoursHoursCacheData[$month['date_name']])) {
                            ...
                            if ($month['date_name'] < '2026-01') {
                                $hours = WorkhourPivot::where('date', $month['date_name'])->where('is_main', true)->where('code', $object->code)->sum('hours');
                            } else {
